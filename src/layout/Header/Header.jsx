...
// 1. Виносимо функцію підрахунку за межі компонента.
// Це захищає від її перестворення при кожному рендері.
const getCartCount = () => {
  if (typeof window === "undefined") return 0; // Захист для SSR (якщо використовується)
  const savedCart = JSON.parse(localStorage.getItem("pizza_cart")) || [];
  return savedCart.reduce((sum, item) => sum + item.quantity, 0);
};

const Header = () => {
  // 2. Ініціалізуємо стан правильним значенням одразу при старті
  const [cartCount, setCartCount] = useState(() => getCartCount());
  const [isMenuOpen, setIsMenuOpen] = useState(false);

  useEffect(() => {
    // 3. Створюємо обробник подій, який оновлює стан
    const handleUpdate = () => {
      setCartCount(getCartCount());
    };

    // Слухаємо події оновлення кошика
    window.addEventListener("cartUpdate", handleUpdate);
    window.addEventListener("storage", handleUpdate);

    // Прибираємо слухачі при видаленні компонента
    return () => {
